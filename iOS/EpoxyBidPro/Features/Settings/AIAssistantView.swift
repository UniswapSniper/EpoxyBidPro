...
// ─── AI Assistant ────────────────────────────────────────────────────────────
// Chat-based AI assistant for epoxy floor business coaching.
// Extracted from MoreView for use in SettingsSheet navigation.
...
protocol AIAssistantServing {
    func reply(to message: String, mode: AssistantMode, tone: AssistantTone, context: AssistantContextPayload?) async throws -> String
}
...
struct AIAssistantService: AIAssistantServing {
    func reply(
        ...
        request.httpBody = try JSONEncoder().encode(
            AssistantRequest(message: message, mode: mode, tone: tone, context: context)
        )
    ...
    private func localFallback(for message: String) -> String {
        ...
        if lower.contains("bid") || lower.contains("quote") {
            return "Open Pipeline → Bids, run Scan Space, then Build From Scan to reduce manual entry and tighten pricing consistency."
        }
